...
"""
统计 Driving 四通道 future_interaction_grid 标签。

支持检查：
1. measurements 与 driving_future_interaction_grids 的文件覆盖率；
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="统计 Driving future_interaction_grids 四通道标签质量。"
    )
    parser.add_argument(
        "--input",
        type=Path,
        required=True,
        help=(
            "数据集根目录、单条route目录，或driving_future_interaction_grids目录。"
        ),
    )
    parser.add_argument(
        "--grid-folder",
        type=str,
        default="driving_future_interaction_grids",
        help="结构化未来世界标签文件夹名称。",
    )
    parser.add_argument(
        "--lg-folder",
        ...
        help="measurement文件夹名称，用于统计帧覆盖率。",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("./driving_future_interaction_grid_statistics"),
        help="统计结果保存目录。",
    )
    parser.add_argument(
        "--downsample-sizes",
