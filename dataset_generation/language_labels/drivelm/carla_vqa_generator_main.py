...
def parse_arguments():
    ...
    path_group.add_argument('--data-directory', type=str, default='database/simlingo_v2_2026_07_21_16_16_03',
                            help='Data directory containing the dataset')  # 数据集根目录
    path_group.add_argument('--output-directory', type=str, default='database/simlingo_v2_2026_07_21_16_16_03/drivelm',
                            help='Output directory for the vqa-graph')     # VQA graph 保存位置
    path_group.add_argument('--output-graph-examples-directory', type=str, default='database/simlingo_v2_2026_07_21_16_16_03/drivelm',
    ...
    img_group.add_argument('--original-fov', type=float, default=110,
                           help='Original field of view')                # 原始视场角
    ...
    viz_group.add_argument('--save-examples', action='store_true', default=False,
                           help='Save example images')  # 保存示例图像
    viz_group.add_argument('--visualize-projection', action='store_true', default=False,
                           help='Visualize object centers & bounding boxes in the image')  # 在保存的示例图像上绘制b_box的点
    ...
    viz_group.add_argument('--skip-existing', action='store_true', default=False,
                           help='Skip existing files when saving examples')   # 保存示例时跳过现有文件,默认default=True(此时运行时不添加这个参数的话,检测到生成了文件的话就自动跳过不再生成)
...
if __name__ == '__main__':
    ...
    len_data_boxes = len(qas_generator.data_boxes_paths)  # 30个

    if multi_processing:  # 不执行
