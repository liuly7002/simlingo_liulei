...
class QAsGenerator():
    ...
    def __init__(self, args):
        ...
        self.TARGET_IMAGE_SIZE = args.target_image_size      # 目标图像尺寸(1024x384)
        self.ORIGINAL_IMAGE_SIZE = args.original_image_size  # 原始图像尺寸(1024x512)
        self.ORIGINAL_FOV = args.original_fov                # 原始视场角(110度)

        # Region of interest (ROI) for image projection
        self.MIN_X = args.min_x
        self.MAX_X = args.max_x
        self.MIN_Y = args.min_y          # ROI(用于切割底部部分)的最小Y坐标
        ...
        self.save_examples = args.save_examples                              # 保存示例图像
        self.visualize_projection = args.visualize_projection                # 在保存的示例图像上绘制b_box的点
        ...
        self.data_boxes_paths_all = glob.glob(os.path.join(self.data_directory, 'data/simlingo/*/*/*/*/boxes/*.json.gz'))  # 只取全部.json.gz文件的前30个文件的话就在后面添加[:30]
    ...
    def create_qa_pairs(self, path_id):
        ...
        # 依次为:
        #   示例(path)             : database/simlingo_v2_2026_02_28/data/simlingo/training_1_scenario/routes_training/random_weather_seed_1_balanced_150/Town12_Rep0_532_route0_02_28_11_05_28/boxes/0032.json.gz
        ...
        if 'lb1_split' in path_measurements:
            ...
            try:
                ...
                # 示例(scenario_name): InterurbanActorFlow
            except Exception as e:
                print(f"Error in {path_measurements}: {e}")
                return
            route_file_number = re.search(rf'Rep*(\d+)_*(\d+)_route_*(\d+)', path_measurements).group(2)  # 532: 这是采集数据的路径的.xml文件名
        ...
        if not os.path.exists(path):   # 如果box文件不存在,直接跳出
            return
        ...
        if self.filter_routes_by_result:
            ...
            # results_file=database/simlingo_v2_2026_02_28/data/simlingo/training_1_scenario/routes_training/random_weather_seed_1_balanced_150/Town12_Rep0_532_route0_02_28_11_05_28/results.json.gz
            try:
            ...
            other_infractions_than_min_speed_infractions_exist = False  # 其他违规行为（非最低速度违规）
            for key in route_results['infractions'].keys():
                if len(route_results['infractions'][key]) > 0 and key != 'min_speed_infractions': # 存在违规并且不是最低速违规
                    other_infractions_than_min_speed_infractions_exist = True  # 那么认为这是失败案例,直接杀死
                    break
            ...
            if other_infractions_than_min_speed_infractions_exist and \
                        (route_results['scores']['score_composed'] < 98.0 or   # 路线完成率小于98
                        route_results['scores']['score_route'] < 98.0):        # 得分小于98
                return                                                         # 直接跳出
        ...
        with gzip.open(path, 'rb') as f:
            file_content = f.read()
            data = json.loads(file_content.decode('utf-8'))             # box数据！！！

        with gzip.open(path_measurements, 'rb') as f:
            file_content = f.read()
            measurements = json.loads(file_content.decode('utf-8'))     # measurements数据！！！

        # Get perception questions
        image_path = path.replace('boxes', 'rgb').replace('.json.gz', '.jpg')
        relative_image_path = image_path  # 图像相对路径
    ...
    def generate_object_key_value(self, category, visual_description, object_count, 
        ...
        if projected_points is not None:
            object_info['2d_bbox'] = self.generate_2d_box_from_projected_points(projected_points)  # projected_points：每个角点在图像坐标系中的像素坐标(2D) 8×2
            object_info['3d_bbox'] = projected_points_meters.round(1).tolist()  # 将数组中所有数值保留 1 位小数

            mean = np.round(np.mean(object_info['2d_bbox'], axis=0), decimals=1)
            center_x = float(mean[0])   # 2D矩形中心点
            center_y = float(mean[1])   # 2D矩形中心点

        # Generate a unique key for the object
        if projected_points is not None:
            object_key = f"<c{object_count + 1},CAM_FRONT,{center_x},{center_y}>"
            # <c1,CAM_FRONT,512.2,287.8>
        else:
    ...
    def generate_vehicle_information(self, other_vehicles, ego_vehicle, important_objects, key_object_infos, 
        ...
        生成场景中车辆的信息和问答对。
            ...
            - 车辆的行驶方向？            
            - 车辆的行驶状态是什么？           
            - 目标车辆 {command_str} 是否可能与目标车辆 {vehicle_location_description} 交叉？           
            参数：           
            other_vehicles（列表）：包含场景中其他车辆信息的字典列表。          
            ego_vehicle（字典）：包含目标车辆信息的字典。         
            important_objects（列表）：用于存储场景中重要对象的列表。        
            key_object_infos（字典）：用于存储场景中对象信息的字典。        
            num_lanes_ego（整数）：目标车辆的行驶车道数。        
            vehicles_by_id（字典）：将车辆 ID 映射到车辆信息的字典。        
            current_measurement（字典）：包含当前测量数据的字典。        
            scenario（str）：当前场景。        
            返回值：       
            qas_conversation_vehicle（list）：与车辆相关的问答对列表。        
            important_objects（list）：场景中重要对象的更新列表。        
        ...
        def determine_path_crossing(current_measurement, ego_vehicle, other_vehicle_location_description, 
            ...
            """
            """
            答案：“自车 {command_str}。{vehicle_location_description} 是否可能与自车交叉？”
                参数：               
                current_measurement (dict)：当前测量数据。                
                ego_vehicle (dict)：自车信息。            
                other_vehicle_location_description (str)：其他车辆位置描述。              
                other_vehicle (dict)：其他车辆信息。              
                vehicles_by_id (dict)：按 ID 分类的车辆字典。              
                other_vehicle_description (str)：其他车辆描述。               
                scenario (str)：当前场景。               
                ego_distance_to_junction (float)：自车到下一个路口的距离。              
                other_vehicle_points_towards_ego (bool)：如果其他车辆指向自车，则为 True。             
                other_vehicle_heading_angle_deg (float): 另一辆车的航向角（度）。              
                pointing_towards_junction (bool): 如果另一辆车正朝着路口方向行驶，则为 True。               
                is_ego_on_highway (bool): 如果本车在高速公路上行驶，则为 True。              
                is_ego_in_accel_lane (bool): 如果本车在加速车道上行驶，则为 True。               
                is_other_veh_in_accel_lane (bool): 如果另一辆车在加速车道上行驶，则为 True。              
            ...
            command_map = {
                1: 'turns left at the next intersection',       # 下个路口左转
                2: 'turns right at the next intersection',      # 下个路口右转
                3: 'drives straight at the next intersection',  # 下个路口直行
                4: 'follows the road',                          # 车道保持
                5: f'does a lane change to the left in {int(command_distance)} m',  # 左变道
                6: f'does a lane change to the right in {int(command_distance)} m', # 右变道
            }
        ...
        def determine_other_vehicle_position(other_vehicle_location_description, other_vehicle, ego_vehicle,
            ...
            答案：“{other_vehicle_location_description} 位于道路上的哪个位置？”
                ...
                is_ego_on_highway (bool): 如果自我车辆在高速公路上，则为 True。       
                is_ego_in_accel_lane (bool): 如果自我车辆在加速车道上，则为 True。                
                is_ego_in_exit_lane (bool): 如果自我车辆在出口车道上，则为 True。               
                other_vehicle_description (str): 另一辆车的描述。              
                is_ego_in_entry_lane (bool): 如果自我车辆在入口车道上，则为 True。             
                ego_about_to_exit_highway (bool): 如果自车即将驶出高速公路，则返回 True。              
                scenario (str): 当前场景。             
                qas_conversation_vehicle (list): 车辆的问答对列表。             
                返回值：               
            ...
            if -135 < orientation_relative_to_ego < -45:   # 指向左侧
                orientation_str = 'is pointing leftwards'
            elif 45 < orientation_relative_to_ego < 135:   # 指向右侧
                orientation_str = 'is pointing rightwards'
            elif 135 < orientation_relative_to_ego or orientation_relative_to_ego < -135:  # 面向自车
                orientation_str = 'is pointing towards the ego vehicle'
            else:
                orientation_str = 'is pointing in the same direction as the ego vehicle'   # 与自车同向

            # Handle cases where the other vehicle is in a junction or on another entry of the junction
            # 1. 处理另一辆车位于路口或路口另一入口的情况。
            ...
            # 4. 处理两辆车均不在路口且都在同一条道路上的情况
            elif not other_vehicle['is_in_junction'] and same_road:
                value = int(other_vehicle['lane_relative_to_ego']) # 与自车的相对车道位置 0：同车道 -1：左侧 1：右侧
        ...
        ego_distance_to_junction = ego_vehicle['distance_to_junction']  # 自车到最近路口的距离（单位米）
        ...
        speed_limit_kmh = int(current_measurement['speed_limit'] * 3.6)  # 限速

        # Flags to indicate if the ego vehicle is in an acceleration lane, exit lane, or entry lane
        # 指示车辆当前位于加速车道、出口车道还是入口车道的标志
        is_ego_on_highway = False         # 在高速
        is_ego_in_accel_lane = False      # 自车在加速车道
        is_other_veh_in_accel_lane = False# 周围车在加速车道
        is_ego_in_exit_lane = False       # 在出口
        is_ego_in_entry_lane = False      # 在入口
        ...
        highway_scenario_names = [
            "EnterActorFlow",          # 进入车流
            "EnterActorFlowV2",        # 进入车流V2
            "HighwayCutIn",            # 高速切入
            "HighwayExit",             # 高速出口
            "MergerIntoSlowTraffic",   # 并入慢速车流
            "MergerIntoSlowTrafficV2", # 并入慢速车流V2
            "YieldToEmergencyVehicle", # 让行紧急车辆
        ]

        # Checks depend on scenario type and set flags accordingly
        # 根据当前 scenario 类型 + 地图结构 + ego 状态，
        # 推断 ego 是否在高速公路，以及处于入口/加速/出口等哪种车道阶段。
        # 规则式高速语义标签生成器
        if scenario == "HighwayCutIn":
            is_ego_on_highway = True
            if ego_vehicle['is_in_junction'] or ego_distance_to_junction < 25:
                is_other_veh_in_accel_lane = True  # 认为其他车辆在加速车道（acceleration lane）切入。
        elif scenario == "HighwayExit" or scenario == "MergerIntoSlowTrafficV2":   # 高速出口场景或慢车流合流场景
            is_ego_on_highway = True
            if ego_vehicle['is_in_junction'] or ego_distance_to_junction <25:
                is_ego_in_exit_lane = True
            if ego_vehicle['num_lanes_same_direction'] - ego_vehicle['ego_lane_number'] - 1 == 0 and \
                        current_measurement['command'] == 6 and ego_distance_to_junction < 40:  #ego 在同方向车道的最外侧 当前规划器要求驶离高速
            ...
            if scenario == 'MergerIntoSlowTraffic' and ego_vehicle['num_lanes_same_direction'] == 1 and \
                                                                    ego_vehicle['num_lanes_opposite_direction'] == 1:  # 单车道双向
                is_ego_in_entry_lane = True   # 是入口车道
                is_ego_in_accel_lane = False  # 不是加速车道
            elif scenario == 'MergerIntoSlowTraffic' and ego_vehicle['num_lanes_same_direction'] > 1:  # 多车道
                is_ego_in_entry_lane = False
                is_ego_in_accel_lane = False
            elif ego_vehicle['is_in_junction'] or ego_distance_to_junction <25:
                is_ego_in_accel_lane = True   # 接近高速入口 junction → 判定为加速车道。
            elif ego_vehicle['num_lanes_same_direction'] == 1 and ego_vehicle['num_lanes_opposite_direction'] == 0:
                is_ego_in_entry_lane = True  # 典型高速匝道
        ...
        for vehicle in other_vehicles:
            ...
            # important_object_str = the black car nearby to the front of the ego vehicle
            ...
            # vehicle_location_description = the black car that is nearby to the front of the ego vehicle
            ...
            key, value = self.generate_object_key_value(
                ...
                object_count = len(key_object_infos),
                projected_points = projected_points,               # 边框8角点在图像中位于什么像素坐标
                projected_points_meters=projected_points_meters    # 边框8角点位于自车坐标系(x向前y向右z向上)中的什么位置
            )
    ...
    def generate_perception_questions(self, scene_data, measurements, scenario):
        ...
        num_objects（整数）：场景中的对象总数。
        num_questions_per_category（字典）：包含每个类别问答数量的字典。
        key_object_infos（字典）：包含场景中对象信息的字典。
        """
        ...
        for actor in scene_data:
            if actor['class'] == 'ego_car':       # 自车
                ego_vehicle = actor
            elif actor['class'] == 'car':         # 周围车辆的信息
        ...
        num_questions = len(qas_conversation_vehicle) + len(qas_conversation_roadlayout) + \
                        ...
                        len(qas_conversation_ego)  # 问题的数量
        ...
        num_questions_per_category = {
            'dynamic_vehicles': len(qas_conversation_vehicle) // 2,          # 动态周围车辆
            'roadlayout': len(qas_conversation_roadlayout) // 2,             # 道路布局
            'stopsign': len(qas_conversation_stopsign) // 2,                 # 停止信号
            'trafficlight': len(qas_conversation_trafficlight) // 2,         # 交通信号灯
            'pedestrian': len(qas_conversation_pedestrian) // 2,             # 行人
            'ego': len(qas_conversation_ego) // 2,                           # 道路拓扑信息
        }
